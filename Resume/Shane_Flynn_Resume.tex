%=================20==================40=================60===================80
%                          User Packages/Definitions
%==============================================================================%
\documentclass[letterpaper]{article}
\usepackage{hyperref}
\usepackage{geometry}
%==============================================================================%
\def\name{Shane W. Flynn}
\def\footerlink{}
%==============================================================================%
%                                PDF Metadata
%==============================================================================%
\hypersetup{
  colorlinks = true,
  urlcolor = black,
  pdfauthor = {\name},
  pdfkeywords = {},
  pdftitle = {\name: Curriculum Vitae},
  pdfsubject = {Curriculum Vitae},
  pdfpagemode = UseNone
}
%==============================================================================%
\geometry{
  body={6.5in, 10in},
  left=1.0in,
  top=0.5in
}
%==============================================================================%
%                                Page Headers
%==============================================================================%
\pagestyle{myheadings}
\markright{\name}
\thispagestyle{empty}
%==============================================================================%
%                               Section Fonts
%==============================================================================%
\usepackage{sectsty}
\sectionfont{\rmfamily\bfseries\large}
\subsectionfont{\rmfamily\bfseries}
\setlength\parindent{0pt}
%==============================================================================%
%                              Begin Document
%==============================================================================%
\begin{document}
\centerline{\huge \textbf{\name}}
\vspace{0.1in}
%==============================================================================%
%                              Contact Information
%==============================================================================%
University of California Irvine \hfill shane.flynn001@gmail.com \\
my-address \hfill GitHub: \url{https://github.com/swflynn}\\
my-phone-number \hfill LinkedIn: \url{https://www.linkedin.com/in/shane-flynn-676b72184/}
\hrule
\vspace{0.15in}
%==============================================================================%
%                              Experience
%==============================================================================%
{\large \textbf{\underline{Experience}}}\\
\vspace{-0.1in}\\
\textbf{Graduate Researcher Theoretical Chemistry}


%\textbf{Graduate Researcher Theroretical Chemistry}


%\textbf{UnderGraduate Researcher Theroretical Chemistry}

\vspace{0.1in}
%==============================================================================%
%                     Projects and Technical Skills
%==============================================================================%
{\large \textbf{\underline{Projects and Technical Skills}}}\\

\textbf{Efficient sampling of any multi-dimensional distribution function.}
\hfill 2018-2020
\begin{itemize}
    \item Derived a novel method for efficient sampling of a general
    multi-dimensional distribution function.
    \item Developed Fortran and Python implementations sampling model
    distributions as proof of principle.
    \item Demonstrated the superior efficiency of the method in the context of
    grid-based methods for solving the generalized eigenvalue problem.
%    \item \textbf{Shane W. Flynn}, Vladimir A. Mandelshtam,
%          ``Sampling general distributions with quasi-regular grids: Application
%           to the vibrational spectra calculations''
%           \textit{J. Chem. Phys. Comm.} \textbf{2019} 151(24) p.241105
    \item \url{https://github.com/swflynn/Quasi_Regular_Grids}
\end{itemize}

%\textbf{English-Mandarin Flashcard App.}
%\hfill Jan 2020
%\begin{itemize  }
%    \item Developed an English-Mandarin Flashcard App for drilling vocabulary
%    and pronunciation.
%    \item Machine learning implementations to customize user experience.
%\end{itemize}

%==============================================================================% %                                 Education
%==============================================================================%
{\large \textbf{\underline{Education}}}\\

\textbf{Ph.D  Theoretical Chemistry}, University of California Irvine.
\hfill 2017-2021\\
Research Advisor: Vladimir A. Mandelshtam \null \hfill (Expected)\\
\vspace{0.1in}

\textbf{M.S. Theoretical Chemistry}, California Institute of Technology.
\hfill 2015-2017 \\
Research Advisor: William A. Goddard III\\
%GPA: 3.7/4.3
\vspace{0.1in}

\textbf{B.S. Chemistry. B.S. Biology. Minor Math}, University of
Massachusetts Boston. \hfill 2010-2015\\
Research Advisors: Jason R. Green. Steven M. Ackermann\\
%GPA: 3.7/4.0
%==============================================================================% %                                  Skills/Tools
%==============================================================================%

{\large \textbf{\underline{Skills}}}\\
Python, Fortran, Java, JavaScript, SQL, NoSQL, Hadoop, Hive, Scala, Spark, Git, Tableau
\end{document}
